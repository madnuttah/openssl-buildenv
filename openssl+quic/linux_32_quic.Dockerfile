...
RUN set -xe; \
  ...
  # determine OPENSSL_VERSION: prefer build-arg, otherwise fetch latest release (fallback to first tag)
...
RUN set -xe; \
  ...
  # determine NGTCP2_VERSION: prefer build-arg, otherwise fetch latest release (fallback to first tag)
